...
gnome-terminal -- bash -c "cd task-processing-system && make run-config; exec bash" &

echo "Starting Rust server in new terminal:"
gnome-terminal -- bash -c "cd task-processing-system-rs && RUST_LOG=info cargo run; exec bash" &
...
echo "Running unit tests in C++"
cd task-processing-system
make test-all || {echo "Tests in C++ failed"; exit 1;
cd ..

echo "Running unit tests in Rust"
cd task-processing-system-rs
cargo test || {echo "Tests in Rust failed"; exit 1;}
cd ..

echo "Running performance test: C++ vs Rusty"
